...
function messageRecieved(msg) {
    try {
        ...
        if (msg.sender != "twitter.js") return;
        ...
        switch (msg.intent) {
            case 'update_init':
                ...
                http_req_servertime.onload = function () {
                    ...
                    let server_update_time = Number(http_req_servertime.responseText.split("<twitter>")[1].split("</twitter>")[0]);
                    chrome.storage.local.get(function(storage_res) {
                        let local_update_time = storage_res.twitter_update_time;
				    ...
				    console.log("in second request");
                                ...
                                let close_modal = function (div, acknowledged) {
                                    ...
                                    if (acknowledged) chrome.storage.local.set({'twitter_update_time': Math.floor((new Date).getTime()/1000)}, function() { console.log("acknowledged!"); });
                                    else chrome.storage.local.set({'twitter_update_time': 0}, function() { });
                                };
                            ...
                            http_req_modalcontent.open("GET", "https://raw.githubusercontent.com/JakeGuy11/image-sourcerer/main/startup-scripts/messages/twitter.html");
                ...
                break;
            case 'toggle_enabled':
                // Get the state of twiter_enabled
                let twitter_is_enabled = false;
                chrome.storage.local.get(function(result) { twitter_is_enabled = result.twitter_enabled; });
                if (twitter_is_enabled == undefined) twitter_is_enabled = false;

                // Toggle the state, write it
                twitter_is_enabled = !twitter_is_enabled;
                chrome.storage.local.set({'twitter_is_enabled': twitter_is_enabled}, function() { });
                break;
            default:
                // Do nothing
                break;
