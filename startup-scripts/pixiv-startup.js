        ...
        switch (msg.intent) {
            case 'update_init':
                ...
                http_req_servertime.onload = function () {
                    ...
                    let server_update_time = Number(http_req_servertime.responseText.split("<pixiv>")[1].split("</pixiv>")[0]);
                    chrome.storage.local.get(function(storage_res) {
                        let local_update_time = storage_res.pixiv_update_time;
				    ...
				    console.log("in second request");
                                ...
                                let close_modal = function (div, acknowledged) {
                                    ...
                                    if (acknowledged) chrome.storage.local.set({'pixiv_update_time': Math.floor((new Date).getTime()/1000)}, function() { console.log("acknowledged!"); });
                                    else chrome.storage.local.set({'pixiv_update_time': 0}, function() { });
                                };
                ...
                break;
            case 'toggle_enabled':
                // Get the state of pixiv_enabled
                let pixiv_is_enabled = false;
                chrome.storage.local.get(function(result) { pixiv_is_enabled = result.pixiv_enabled; });
                if (pixiv_is_enabled == undefined) pixiv_is_enabled = false;

                // Toggle the state, write it
                pixiv_is_enabled = !pixiv_is_enabled;
                chrome.storage.local.set({'pixiv_init': pixiv_is_enabled}, function() { });
                break;
            default:
                // Do nothing
                break;
